...
- `NexusAPI-Evaluator-macOS-x64.zip`：包含 `NexusAPI Evaluator.app` 和运行资源。
- `NexusAPI-Evaluator-Windows-x64.zip`：包含 `NexusAPI Evaluator.exe` 和运行资源。
...
普通交付包会设置 `NEXUSAPI_ENABLE_SAFETY_SCENARIOS=0`，隐藏内部内容安全合规测试。
...
4. 点击 `Run workflow`。
5. 等 macOS 和 Windows 两个任务都通过后，下载 artifacts 里的 zip 包。

Release 打包：

1. 创建并推送 tag，例如 `v0.1.0`。
2. GitHub Actions 会自动打包 macOS 和 Windows。
