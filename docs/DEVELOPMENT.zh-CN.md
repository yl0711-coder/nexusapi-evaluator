...
- `NexusAPI数据/日志/errors.jsonl`：技术错误日志，保存内部错误、前端错误和任务失败细节；前端只展示错误编号和小白可理解的提示。
...
- 普通版使用 `pnpm dev:desktop:standard` 或 `pnpm tauri:build:standard`，会通过 `NEXUSAPI_ENABLE_SAFETY_SCENARIOS=0` 关闭内容安全场景，避免外包人员误用。
...
1. 添加或确认 API 配置。
2. 先跑一次快速测试。如果快速测试失败，不要继续跑稳定性测试。
3. URL、模型名、Key 或协议变更后，先跑 3 轮冒烟测试。
4. 常规评估跑 10 轮基础稳定性测试。
5. 对比候选渠道或准备推荐服务商前，跑 30 轮测试。
6. 多渠道对比时使用批量稳定性测试：
   - `同时测试 API 数` 默认保持 2。
